...
|---|---|---|---|---|
|21|圆括号|（...）|无|最高|
|20|成员访问|obj.prop/arr[i]|从左到右|访问对象属性或数组元素|
|19|函数调用|fn()、obj.method()|从左到右|函数执行调用|
|18|new（带参数）|（...）|无|带括号的构造函数调用（如 new Date()）|
|17|后置递增 / 递减|`a++、a--`|从左到右|先使用变量值，再增减（如 console.log(a++) 输出原始值）|
|16|逻辑非|`!a`|从右到左|逻辑取反|
|16|按位非|`~a`|从右到左|逐位取反|
|16|一元加 / 减|`+a -a`|从右到左|转换为数字（`+ "123" → 123`）或取反（`-5 → -5`）|
|16|前置递增/递减|`++a --a`|从右到左|先增减变量值，再使用|
|16|typeof|`typeof a`|从右到左|检测变量类型|
|16|void|`void a`|从右到左|执行表达式但返回 undefined（如 void (a = 1) 等价于 a=1; undefined）|
|15|new（无参数）|`new Fn`|从右到左|不带括号的构造函数调用（如 new Object）|
|14|乘法 / 除法 / 模|`* / %`|从左到右|算数运算|
|13|加法 / 减法|`+ -`|从左到右|算术运算或字符串拼接（"a" + "b" → "ab"）|
|12|移位|`<< >> >>>`|从左到右|二进制移位（`8 >> 1 → 4`）|
|11|关系运算符|`< > <= >= in instanceof`|从左到右|比较大小或归属（`3 > 2 → true，"a" in {a:1} → true`）|
|10|相等运算符|`=== != === !==`|从左到右|判断相等`(1=='1'→ true) 1 === "1" → false`|
|9|按位与|`&`|从左到右|二进制与运算（`3 & 5 → 1`）|
|8|按位异或|`^`|从左到右|二进制异或运算`3 ^ 5 → 6`|
|7|按位或|`｜`|从左到右|二进制与运算（`3 ｜ 5 → 7`）|
|6|逻辑与|`&&`|从左到右|短路逻辑（`a && b：a 为假则返回 a，否则返回 b`）|
|5|逻辑或|`｜｜`|从左到右|逻辑（`a ｜｜ b：a 为假则返回 b，否则返回 a`）|
|4|条件运算符|`a ? b : c`|从右到左|三目运算（`age > 18 ? "成年" : "未成年"`）|
|3|赋值运算符|`= += -= *=`|从右到左|赋值（`a = b = 1 等价于 a = (b = 1)`）|
|2|yield|`yield yield*`|从右到左|生成器函数中返回值（特殊场景）|
|1|逗号运算符|`,`|从左到右|分隔表达式，返回最后一个值（`a=1, b=2, a+b → 3`）|

```js
// # 题目 
[] == ![] // true 解释：![] 中，[] 作为对象转为布尔值是 true，取反后为 false；[] 与 false 比较时，[] 转原始值为 ""（空字符串），再转数字为 0；false 转数字为 0，故 0 == 0。
3 + 2 * 2 // 7
true && false || true // true 
a = 1; ++a + a++（执行后结果是多少）//4
'5' + 3 与 5 + '3' //53 解释：均为字符串拼接
3 & 5 与 3 && 5 // 1 5
null == undefined 与 null === undefined // true false
a = b = 3; a += b *= 2（a 和 b 最终值） // a = 9, b = 6
1 < 2 < 3 与 3 > 2 > 1 // true false
typeof typeof 123 // 'string'
!typeof null // false
0 || 1 && 2 // 2
(a = 2, b = 3, a + b) // 5
~-1 // 0 运算为 -(x+1)
a = {n: 1}; b = a; a.x = a = {n: 2};（a.x 和 b.x 的值） // a.x为undefined， b.x为{n:2} 解析：a.x 赋值时，左侧 a 仍指向原始对象（b 也指向它），右侧 a 被重新赋值为新对象 {n:2}；最终原始对象的 x 是新对象（b.x），新对象 a 无 x 属性（a.x 为 undefined）。
```
...
模块化是前端工程化的基础，核心价值在于解决代码规模化带来的管理问题，具体表现为：
...
模块化规范对比
|规范|推出时间|适用场景|加载方式|核心语法（导出 / 导入）|优点|缺点|
|---|---|---|---|---|---|---|
|IIFE|早期|浏览器（无模块环境）|同步执行|用立即执行函数封装，通过全局变量暴露接口 |简单直接，解决全局污染|无官方规范，依赖管理混乱，无法按需加载|
|CommonJS|2009| Node.js（服务器端）|同步加载 |导出：module.exports/exports；导入：require() |适合服务器端（文件读取同步），npm 生态支持|同步加载不适合浏览器（阻塞渲染）|
|AMD|2010| 浏览器（异步场景）|异步加载 |导出：define 定义模块；导入：require 异步加载 |支持浏览器异步加载，适合依赖复杂的场景|语法冗余，书写复杂|
|UMD|2011| 通用（浏览器 + Node）|按需适配 |兼容 CommonJS/AMD/ 全局变量，自动适配环境 |跨环境兼容，适合第三方库发布|代码冗余，本质是 “兼容层” 而非新规范|
|ESM|2015| 现代浏览器 + Node.js| 同步 / 异步 |导出：export/export default；导入：import |官方标准，静态分析，支持树摇、按需加载|旧浏览器不支持（需转译），Node 需显式声明|

### 总结
- 模块化的核心是 “封装 + 复用 + 依赖管理”，解决了大规模代码的维护问题。
- 演进趋势：从早期的 IIFE（伪模块化）→ 环境特定规范（CommonJS/AMD）→ 通用兼容（UMD）→ 官方标准（ESM）。
- 现状：ESM 是现代前端的首选，配合 Webpack/Vite 等工具可兼容其他规范，兼顾开发效率和性能。