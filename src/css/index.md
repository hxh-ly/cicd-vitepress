...
|---|---|---|
|主要运行线程|合成器线程|主线程|
|与渲染管线关系|可避免布局（layout）和绘制(paint)|通常在主线程上触发完整的渲染管线|
|性能关键|依赖浏览器动画的优化，特别是当只修改特定属性（如transform、opacity），可直接由GPU执行|依赖代码执行效率，若主线程任务繁重导致动画卡顿，使用requestAnimationFrame可改善同步性|
|控制能力|较弱，难以在动画过程中进行精确控制（如添加回调）|极强，可精确控制每一帧，实现复杂动画逻辑（如暂停、反转、动迹）|
|典型应用场景|简单的状态切换（如悬停效果）、视图转换等|复杂等交互动画（如游戏、数据可视化、滚动动画）|